...
async def test_get_tools_vs_get_tools_with_prefix():
  ...
  # Now calling get_tools_with_prefix should modify the tool names in place
  prefixed_tools = await toolset.get_tools_with_prefix()
  assert len(prefixed_tools) == 2
...
@pytest.mark.asyncio
async def test_double_prefixing_avoided():
  """Test that calling get_tools_with_prefix multiple times doesn't double-prefix."""
  tool1 = _TestingTool(name='tool1', description='Test tool 1')
  toolset = _TestingToolset(tools=[tool1], tool_name_prefix='test')

  # First call should prefix the tool
  prefixed_tools_1 = await toolset.get_tools_with_prefix()
  assert len(prefixed_tools_1) == 1
  assert prefixed_tools_1[0].name == 'test_tool1'

  # Second call should not double-prefix
  prefixed_tools_2 = await toolset.get_tools_with_prefix()
  assert len(prefixed_tools_2) == 1
  assert (
      prefixed_tools_2[0].name == 'test_tool1'
  )  # Should still be test_tool1, not test_test_tool1
